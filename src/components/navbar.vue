<template>
  <header :class="!isMobile && 'fixed-header'">
    <div v-if="loadBar && !isMobile" class="loading-bar">
      <div class="progress-bar"></div>
    </div>
    <nav
      class="navbar space-around align-center"
      :class="isMobile ? 'red-background' : 'white-background py-15'"
    >
      <div class="container space-around align-center">
        <router-link class="content-center" to="/">
          <div :class="isMobile ? 'logo-mobile' : 'logo-desktop'">
            <img v-if="isMobile" src="@/assets/yt_icon_mono_dark.png" alt="" />
            <svg
              v-else
              viewBox="0 0 90 20"
...
export default {
  ...
  watch: {
    ...
    searchText(val) {
      val && this.$router.push(`/${this.query}`);
    },
  },
  data() {
    return {
...
#search-input-mobile[type="search"] {
  padding: 5px 15px;
  width: 80%;
  color: #000;
